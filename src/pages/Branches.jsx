      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {isLoading ? (
           [1, 2, 3].map(i => (
            ...
            branches.map(branch => (
              <div key={branch._id} className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden flex flex-col">
                 ...
                 <div className="flex items-start justify-between mb-4 relative z-10">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <span className="inline-flex items-center gap-1 px-2 py-1 bg-brand-50 text-brand-700 text-xs font-bold rounded-md border border-brand-100">
                          <ShieldCheck size={12} /> {branch.branchCode}
                        </span>
                        <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${branch.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                ...
                <div className="space-y-3 relative z-10 flex-1">
                 ...
                 <div className="flex items-center gap-2 mt-4 pt-4 border-t border-slate-100">
                     <button onClick={() => handleToggleStatus(branch)} className={`flex-1 py-2 rounded-lg border text-[10px] font-bold uppercase tracking-wider transition-colors ${branch.isActive ? 'bg-amber-50 border-amber-200 text-amber-700 hover:bg-amber-100' : 'bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100'}`}>
                        {branch.isActive ? 'Deactivate' : 'Activate'}
                     </button>
                     <button onClick={() => { setFormData({ ...branch }); setIsModalOpen(true); }} className="flex-1 py-2 bg-slate-50 border border-slate-200 rounded-lg text-[10px] font-bold uppercase tracking-wider text-slate-600 hover:bg-brand-50 hover:border-brand-200 hover:text-brand-700 transition-colors">
                        Edit
                     </button>
                     <button onClick={() => handleDelete(branch)} className="flex-1 py-2 bg-slate-50 border border-slate-200 rounded-lg text-[10px] font-bold uppercase tracking-wider text-slate-600 hover:bg-red-50 hover:border-red-200 hover:text-red-700 transition-colors">
                        Delete
                     </button>
                 </div>
              </div>
